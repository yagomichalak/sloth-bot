...
class Tools(commands.Cog):
	...
	async def on_message(self, message) -> None:
		...
		announcement_channels: List[int] = [
			576793212304228352, 761303171833790464, 760958206683643975,
			934466647513768016, 784499538366824488, 890621139297128548, 
			871785544361840690, 902585958208254002, 801514509424525363,
			799394160096444456, 906862788293587004, 870437619522236427,
			919993838217613362
		]
	...
	@commands.check_any(utils.is_allowed([*allowed_roles, analyst_debugger_role_id, event_manager_role_id], throw_exc=True), utils.is_subscriber())
	...
	@utils.is_allowed([owner_role_id, admin_role_id, mod_role_id, analyst_debugger_role_id], throw_exc=True)
