...
class CreateSmartRoom(commands.Cog):
	...
	@tasks.loop(hours=3)
	async def check_galaxy_expiration():
	...
	async def premium_room(self, member):
		...
		if str(reaction.emoji) == '✅':
			...
			user_currency = await SlothCurrency.get_user_currency(ctx, member.id)
			if user_currency:
				if user_currency[0][1] >= 100:
					await SlothCurrency.update_user_money(ctx, member.id, -100)
				else:
					return await ctx.send("**You don't have enough money to buy this service!**")

			else:
				return await ctx.send("**You don't even have an account yet!**")
		...
		if str(reaction.emoji) == '✅':
			...
			if user_currency:
				if user_currency[0][1] >= 350:
					await SlothCurrency.update_user_money(ctx, member.id, -350)
				else:
					return await ctx.send("**You don't have enough money to buy this service!**")
	...
	async def delete_premium_vc(self, user_id: int, user_vc: int):
		await mycursor, db = await the_data_base5()
	...
	async def insert_galaxy_vc(self, user_id: int, user_cat: int):
		mycursor, db = await the_data_base5()
