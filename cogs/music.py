...
class Music(commands.Cog):
    ...
    async def skip_(self, ctx):
        ...
        # if commands.has_role("DJ") or YTDLSource.requester:
        #    vc.stop()
        #    if YTDLSource.requester:
        #        await ctx.send(f'**`{ctx.author}`**: The original requester has skipped the song!')
        #    elif commands.has_role("DJ"):
        #        await ctx.send(f'**`{ctx.author}`**: This user has a DJ role and has skipped the song!')
        # elif not commands.has_role("DJ") and not YTDLSource.requester:
