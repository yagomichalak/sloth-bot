...
class Verify(commands.Cog):
    ...
    async def on_raw_reaction_add_verify(self, payload) -> None:
        ...
        # checks if it's in the verification requests channel
        if payload.channel_id != self.verify_reqs_channel_id:
            return

        # check if member is administrator or is a staff manager
        staff_manager_role = discord.utils.get(guild.roles, id=self.staff_manager_role_id)
        is_staff_manager = staff_manager_role in payload.member.roles
        is_admin = payload.member.guild_permissions.administrator

        if not (is_admin or is_staff_manager):
