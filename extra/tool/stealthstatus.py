...
class StealthStatusTable(commands.Cog):
    ...
    async def drop_table_stealth_status(self, ctx) -> None:
        ...
        await ctx.message.delete()
        mycursor, db = await the_database()
        await mycursor.execute("DROP TABLE StealthStatus")
        await db.commit()
        await mycursor.close()
    ...
    async def reset_table_stealth_status(self, ctx):
        ...
        await ctx.message.delete()
        mycursor, db = await the_database()
        await mycursor.execute("DELETE FROM StealthStatus")
        await db.commit()
        await mycursor.close()
    ...
    async def check_table_stealth_status_exists(self) -> bool:
        ...
        await mycursor.execute("SHOW TABLE STATUS LIKE 'StealthStatus'")
    ...
    async def insert_stealth_status(self, user_id: int, state: int = 1) -> None:
        ...
        mycursor, db = await the_database()
        await mycursor.execute("INSERT INTO StealthStatus (user_id, state) VALUES (%s, %s)", (user_id, state))
        await db.commit()
        await mycursor.close()
    ...
    async def get_stealth_status(self, user_id: int) -> List[int]:
        ...
        mycursor, _ = await the_database()
        await mycursor.execute("SELECT * from StealthStatus where user_id = %s", (user_id,))
        user_status = await mycursor.fetchone()
        await mycursor.close()
        return user_status
    ...
    async def update_stealth_status(self, user_id: int, state: int) -> None:
        ...
        mycursor, db = await the_database()
        await mycursor.execute("UPDATE StealthStatus SET state = %s WHERE user_id = %s", (state, user_id))
        await db.commit()
        await mycursor.close()
