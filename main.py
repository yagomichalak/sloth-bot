...
async def on_raw_message_delete(payload):
    if payload.channel_id == announce_channel:
...
async def on_raw_reaction_add(overload):
    ...
    languages = {
        'germanic': [{'native': 687653940602339349, 'fluent': 688070989757808653, 'studying': 688532522052878349},
                     {'🇬🇧': 'english', '🇩🇪': 'german', '🇩🇰': 'danish', '🇳🇱': 'dutch', '🇳🇴': 'norwegian',
                      '🇸🇪': 'swedish', '🇮🇸': 'icelandic', '🇿🇦': 'afrikaans', '🇫🇴': 'faroese',
                      '🇱🇺': 'luxembourgish'}],
        'uralic': [{'native': 687653990791774218, 'fluent': 688071024356360372, 'studying': 688532812797706255},
        ...
        'baltic': [{'native': 687654156852264978, 'fluent': 688072044243714095, 'studying': 688532873312993340},
                   {'🇱🇹': 'lithuanian', '🇱🇻': 'latvian'}],
        'slavic': [{'native': 687654179707027495, 'fluent': 688072145687019524, 'studying': 688532945245307068},
                   {'🇷🇺': 'russian', '🇺🇦': 'ukrainian', '🇵🇱': 'polish', '🇧🇾': 'belarusian', '🇨🇿': 'czech',
                    '🇸🇰': 'slovak', '🇧🇦': 'bosnian', '🇷🇸': 'serbian', '🇸🇮': 'slovenian', '🇭🇷': 'croatian',
                    '🇲🇰': 'macedonian', '🇧🇬': 'bulgarian'}],
        'semitic': [{'native': 687654243653779478, 'fluent': 688072184811618432, 'studying': 688533046894395424},
                    {'🇸🇦': 'arabic', '🇮🇱': 'hebrew', '🇪🇹': 'amharic'}],
        'turkic': [{'native': 687654272284098560, 'fluent': 688072219053785093, 'studying': 688533085306093754},
                   {'🇹🇷': 'turkish', '🇰🇿': 'kazakh', '🇦🇿': 'azerbaijani'}],
        'iranian': [{'native': 687654283684085771, 'fluent': 688073486459207745, 'studying': 123456789},
                    {'🇦🇫': 'pashto', '<:flag_kd:490181096873525258>': 'kurdish', '🇮🇷': 'persian'}],
        'asian': [{'native': 687654312465137679, 'fluent': 688073546835951700, 'studying': 688533171112902663},
                  {'🇯🇵': 'japanese', '🇨🇳': 'chinese', '🇰🇷': 'korean', '🇻🇳': 'vietnamese', '🇹🇭': 'thai',
                   '🇱🇰': 'sinhalese', '🇵🇭': 'tagalog', '🇮🇩': 'indonesian', '🇲🇾': 'malay', '🇲🇳': 'mongolian',
                   '🇭🇰': 'cantonese'}],
        'indian': [{'native': 687654329921830933, 'fluent': 688073575298629788, 'studying': 688533187315499127},
                   {'🇮🇳': 'hindustani'}],
        'unafiliated': [{'native': 688048079697739852, 'fluent': 688073619515113485, 'studying': 688533247986106373},
    ...
    # Class announcement
    if overload.channel_id == announce_channel:
        announcements = await show_class_announcements()
        for announcement in announcements:
            if overload.message_id == announcement[1]:
                if str(overload.emoji) == '✅':
                    channel = discord.utils.get(guild.channels, id=announce_channel)
...
async def repeated(ctx):
    ...
    for teacher1 in teachers1:
        for teacher2 in teachers2:
            if teacher1[0] != teacher2[0] and teacher1[3] == teacher2[3] and teacher1[4] == teacher2[4] and teacher1[
...
async def embed_rules(ctx):
    ...
    embed = discord.Embed(title="Discord’s Terms of Service and Community Guidelines",
                          description="Rules Of The Server", url='https://discordapp.com/guidelines', colour=1406210,
...
@client.command()
async def cmds(ctx):
    embed = discord.Embed(title="Calendar's command list", description="Some useful commands",
                          colour=discord.Colour.dark_green())
    embed.add_field(name='(Admin+) !update_calendar',
                    value='Updates the calendar by organizing each of the available classes in their respective positions.',
                    inline=False)
    embed.add_field(name="(Admin+) !showconfigs", value="Shows the calendar's configuration ids.", inline=False)
    embed.add_field(name="(Admin+) !addconfigs [channel_id] [message_id]",
                    value="Adds the calendar's configuration ids.", inline=False)
    ...
    embed.add_field(name="(Admin+) !spy [channel_id] [message]", value="The bot sends a message to the given channel.",
                    inline=False)
...
async def event_cmds(ctx):
    ...
    embed.add_field(name='(Admin+) !update_event_name [id] [new_name]', value='Updates the name of an existent event.',
                    inline=False)
    embed.add_field(name='(Admin+) !update_event_day [id] [new_day]', value='Updates the day of an existent event.',
                    inline=False)
    embed.add_field(name='(Admin+) !update_event_time [id] [new_time]', value='Updates the time of an existent event.',
                    inline=False)
...
@client.command()
@commands.has_permissions(administrator=True)
async def nclass(ctx, teacher: discord.Member, language, day, time, *desc: str):
    await ctx.message.delete()
    if len(desc) == 0:
        desc = 'Unspecified'
    else:
...
@client.command()
@commands.has_permissions(administrator=True)
async def show_announ(ctx):
    await ctx.message.delete()
    announcements = await show_class_announcements()
    if len(announcements) > 0:
        embed = discord.Embed(title="Class Announcements:", description="__Available class announcements__", colour=discord.Colour.green(), timestamp=ctx.message.created_at)
        for ann in announcements:
...
@client.command()
@commands.has_permissions(administrator=True)
async def del_announ(ctx, class_id: int = None):
    await ctx.message.delete()
    if not class_id:
        return await ctx.send("**Inform the class id!**")
